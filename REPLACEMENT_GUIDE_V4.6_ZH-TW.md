...
不要放成 `/Exhibition-Hub-V4.6-full-replacement/index.html`；這會讓 GitHub Pages 找不到首頁。
...
6. 打開已解壓的 V4.6 替換包，選取裡面的全部檔案與資料夾。
...
10. 回到 GitHub Desktop。左下角 **Summary (required)** 輸入：`Update Exhibition Hub to V4.6`。
...
3. 從已解壓的 V4.6 資料夾，把所有內容拖進「Drag files here」區域。務必包含 `assets`、`data`、`scripts`、`tests` 與 `.github`。
4. 等待所有檔名出現在上傳清單；向下捲到 **Commit changes**。
5. 提交訊息輸入 `Update Exhibition Hub to V4.6`。
...
`data/exhibitions.json` 會由 GitHub Actions 自動更新，因此有時會和本機 V4.6 同時修改。若 GitHub Desktop 顯示 **Resolve conflicts before Merge**，不要手動處理數百或數千個 JSON 區塊，也不要直接刪除 `<<<<<<<`、`=======`、`>>>>>>>`。

### 情況 A：你想保留剛覆蓋的 V4.6 資料
...
7. 若衝突檔是 `data/exhibitions.json` 與／或 `data/geocode-cache.json`，保留本機 V4.6：
...
9. 首頁第一區應有淡暖灰米色滿版背景與極淡紙張網格；主標題應分成 **展覽／是城市寫給你的信** 兩行，副標題必須在 **收錄全台展覽與演出。** 後換行。左側標題、搜尋框與統計資料要比舊版向右，並大致對齊上方導覽內容。右側紙張拼貼要填滿 Hero 的完整高度並一路鋪到瀏覽器右緣，不得局限成置中的小框；四周不得出現外框、圓角或浮起陰影。票券應比 V4.5 略小並靠右，和左側文字保留清楚間距。
...
16. 時間區標題應為 **循著展期，安排下一場相遇**；附近區標題應為 **離你不遠，還有這些選擇。**，說明以 **讓所在的位置成為起點** 開頭。`EDITOR'S SELECTION`、`PLAN YOUR WEEK`、附近區與 `VENUE GUIDE` 上方不得出現過大的空白帶。
...
31. 全站頁面底色應是淡淡淺棕紙張色，而不是接近純白或冷灰；卡片表面應是暖象牙白，導覽列不得和頁面底色產生明顯色差。
32. 進入 **探索展覽**：標題／狀態、分類、日期、地區場館與展覽結果五個大區塊應分別使用低彩度砂岩、鼠尾草、淡陶土及暖灰棕色，並由較深、低亮度的狀態緩慢依序浮現到正常色彩。
33. 探索頁展覽圖卡應由上到下、由左到右依序緩慢浮現。任選一個分類、改變狀態、日期、縣市、場館或排序後，更新後的圖卡必須重新播放一次，不得只在第一次進入頁面時播放。
...
- 強制重新整理；也可用無痕視窗開啟。
- 在網頁原始碼搜尋 `assets/app.js?v=4.6`，確認目前部署的是 V4.6。
...
若錯誤出現在 **Validate frontend, scraper rules, and published data**，並列出 `index_toplogo.png`、`top_icon_2.png` 或 `sharelogo.png`，代表目前執行的仍是 V4.3 舊清洗順序。請完整覆蓋 V4.6，提交 `Update Exhibition Hub to V4.6` 並 Push；接著回到 **Actions → Update data and deploy site → Run workflow → Branch: main → Run workflow**。不必手動編輯 `data/exhibitions.json`，V4.6 會在發布前自動移除這些介面圖片。

若紅字是 `Unable to fetch any exhibition records from the Culture Ministry feeds` 或 `Connection to cloud.culture.tw timed out`，代表文化部服務暫時無回應。V4.6 已加入既有資料備援：它會沿用上一版資料繼續補抓官方頁、測試並部署，不會因單次 API 全部逾時而中止。請先確認程式碼確實已是 V4.6，再按右上角 **Re-run jobs → Re-run all jobs**；若畫面仍顯示舊版錯誤，通常是 V4.6 尚未 Push 到 `main`。

### 顯示 `netloc ... contains invalid characters`

這代表上游單位把網址與中文聯絡資訊黏在同一個欄位，並不是你的 GitHub 操作錯誤。V4.6 已包含網址安全修正，會保留正確網域並捨棄後方說明，不會再讓整批更新中止。覆蓋 V4.6、提交並推送後，再依第七節手動執行一次工作流程即可。
...
2. 點左上方 **History**。
3. 找到 `Update Exhibition Hub to V4.6`。
