# Exhibition Hub V5.0 完整替換指南

這份替換包可直接覆蓋既有 GitHub Pages 專案。ZIP 內第一層就是網站根目錄內容；解壓後請複製「裡面的檔案」，不要把整個 ZIP 外層資料夾再包進儲存庫。

本版已完成：

- Hero 的文字、票券與拼貼位置維持不動，只把「再抽一組觀展靈感」移至拼貼真正的左上角。
- 手機版抽卡按鈕同步移到左上，與第三張票券保留間距，不再遮住票面。
- 「循著位置找展覽」由較深陶米色改為淡燕麥奶色。
- Hero 去除偏紅色調，改為暖白、米黃奶茶與淺木色的中性漸層。
- Hero 內外兩層網格線透明度略微提高，仍維持若隱若現而不搶畫面。
- 日期／狀態展開區使用較深奶茶紙張底，結果圖卡使用較淺的象牙奶茶漸層，增加層次感。
...
| `assets/taiwan-exhibition-journal-logo-v10.png` | `/assets/taiwan-exhibition-journal-logo-v10.png` | 新增或覆蓋 |
...
不要放成 `/Exhibition-Hub-V5.0-full-replacement/index.html`；這會讓 GitHub Pages 找不到首頁。
...
6. 打開已解壓的 V5.0 替換包，選取裡面的全部檔案與資料夾。
...
10. 回到 GitHub Desktop。左下角 **Summary (required)** 輸入：`Update Exhibition Hub to V5.0`。
...
3. 從已解壓的 V5.0 資料夾，把所有內容拖進「Drag files here」區域。務必包含 `assets`、`data`、`scripts`、`tests` 與 `.github`。
4. 等待所有檔名出現在上傳清單；向下捲到 **Commit changes**。
5. 提交訊息輸入 `Update Exhibition Hub to V5.0`。
...
`data/exhibitions.json` 會由 GitHub Actions 自動更新，因此有時會和本機 V5.0 同時修改。若 GitHub Desktop 顯示 **Resolve conflicts before Merge**，不要手動處理數百或數千個 JSON 區塊，也不要直接刪除 `<<<<<<<`、`=======`、`>>>>>>>`。

### 情況 A：你想保留剛覆蓋的 V5.0 資料
...
7. 若衝突檔是 `data/exhibitions.json` 與／或 `data/geocode-cache.json`，保留本機 V5.0：

```bash
git checkout --ours -- data/exhibitions.json data/geocode-cache.json
git add -- data/exhibitions.json data/geocode-cache.json
git status
```
...
```bash
git commit -m "Resolve V5.0 data merge conflict"
```

也可以在第 8 步後回到 GitHub Desktop，點 **Repository → Refresh**。確認兩個 JSON 旁的橘色警告都消失，衝突視窗中的 **Continue Merge** 變成可點擊後：

1. 點右下角 **Continue Merge**。
2. 左下角 **Summary (required)** 輸入 `Resolve V5.0 data merge conflict`。
3. 點 **Commit to main／Commit merge**。
...
7. 工作流程會依序進行全量官方資料補強、執行 49 項測試、上傳網站並部署 Pages。第一次全量更新可能比舊版久，請等待整個 `update-and-deploy` 變成綠色。
...
3. 首頁 Hero：不要碰票券，等待 15 秒應更換一組；把滑鼠停在 Hero 或票券超過 15 秒，票券不得切換；移出後才繼續剩餘倒數。**再抽一組觀展靈感** 必須位於右側紙張拼貼左上角，不可偏到票券中央；點擊後會立即換一組並重新計時。
4. 首頁日期／狀態：選日期，再點目前舉辦、即將舉辦等狀態，日期／狀態的大框應直接向下延伸並在框內顯示圖卡，結果位於分類列上方，不得另外跳出一個獨立區塊。展開區應是比頁面更明顯的淺奶茶底，圖卡則使用較淺的象牙奶茶漸層，兩者必須看得出層次。卡片只會平順浮現，不應閃白或反覆消失。同一個狀態按鈕再點一次，應回到 **全部**。
...
9. 首頁 Hero 主標題應分成 **展覽／是城市寫給你的信** 兩行，副標題必須在 **收錄全台展覽與演出。** 後換行。Hero 的奶茶網格色帶應延伸到瀏覽器左右邊緣，但其中的文字、票券與拼貼位置不可跟著外移；在約 1363px 寬的桌機畫面中，內容左緣仍約在 106px。左側文字還要在內容區內再向右 12px。右側紙張拼貼最大寬度約 680px、高度約 432px，票券最大舞台寬度約 590px。Hero 必須由暖白、米黃奶茶到淺木色由左至右加深，不得帶紅或粉紅感；網格要能看見，但只能若隱若現。
10. 左上角 Logo 只有票券內部是低彩度淺奶油棕，且應比 V4.8 再淺一些；票型、虛線、中文字、英文及其餘透明區域不得改變。整體仍只比旁邊導覽文字稍大。
11. Hero 地圖應移到票券背景左上角、遮住原本的空白區域；只保留四塊低彩度街區、河道、兩條主要路徑、少量節點、兩座建築線稿與定位點，不得出現立體陰影或密集街廓。第二張票券應高於第一張票券頂緣並露出足夠內容；三張票券的中間偏右處都要有直向裁切虛線，並保留雙層類型框、深色條碼與淡色圓章浮水印。
12. 探索區標題應為 **循著今日心緒，遇見一場展覽**；`EXPLORE BY MOOD` 與 Hero 之間仍須能辨識段落，但不得再有過大的空白帶。狀態按鈕懸浮時應顯示淡奶黃色回饋。**查看全部展覽 ↗** 應位於日期／狀態大框最右側；選取條件後，圖卡要在同一外框內由控制列下方向下展開，結果區與圖卡不得再接近純白。
13. 場館區標題應為 **從一座場館，展開城市漫遊**。桌機一次顯示三列、每列四張矮版卡片；進場時不得錯位。滑鼠懸浮時外框必須完全固定，不得上浮、縮放或抖動，只有框內圖片緩慢放大。
14. **本週值得出發的展覽** 圖卡應在原位置淡入揭露，不得先向左錯位；場館圖片失效時應依序切換其他館內展覽圖片，全部失效才顯示精緻分類圖，不得再出現單色大型「館」字。
15. **瀏覽所有場館 ↗** 應位於場館卡片區正下方的中央橢圓按鈕。
16. 時間區標題應為 **循著展期，安排下一場相遇**；附近區標題應為 **這些選擇，離你不遠**，結尾沒有句號，說明以 **讓所在的位置成為起點** 開頭。附近區只需使用比頁面稍明顯的淡奶茶色，不得形成深色或高飽和棕色色帶。`EDITOR'S SELECTION`、`PLAN YOUR WEEK`、附近區與 `VENUE GUIDE` 上方不得出現過大的空白帶。
...
20. 點 **附近展覽**：瀏覽器應詢問定位權限；允許後只顯示 10 公里內活動並由近到遠排列。卡片要有綠色距離標籤與 **地圖導航 ↗**，左側地圖要顯示目前位置、10 公里範圍與活動標記。
...
27. 向下捲動超過首頁 Hero 後，畫面右側應出現小型票券按鈕；點擊後平滑回到頁面最上方。
28. 搜尋 **與稻香共生常態展─台灣穀堡：稻米博物館**：圖片不得再顯示 `ajax-loader.gif` 或永久讀取動畫。
29. 抽查 OPENTIX 活動：網址路徑應為 `/event/活動編號`，不得再使用 `/program/活動編號`；手動資料更新後，原本只有「請前往官方活動頁面」的項目會依序補入官方介紹。
30. 重新整理首頁後，分類、精選、時間、附近與場館動畫應從頭播放；進入 **探索展覽／附近展覽／我的收藏** 再返回首頁，以及切到其他瀏覽器分頁再切回首頁時，也應重新播放。若系統開啟「減少動態效果」，網站會遵循系統設定而不強制播放。
31. 全站頁面底色應是非常淡、低飽和的暖奶茶紙張色，而不是濃棕、黃棕、純白或冷灰；卡片表面應是暖象牙白，導覽列不得和頁面底色產生明顯色差。
32. 進入 **探索展覽**：標題／狀態、分類、日期、地區場館與展覽結果五個大區塊必須能看出不同的極淺暖色，依序為淡陶土、奶油、淺蜜桃、淡粉棕及燕麥色；所有色彩都不得過度飽和。
33. 探索頁所有大區塊與展覽圖卡應立即穩定顯示，不得播放進場、由深至淺或逐張浮現動畫。任選分類、狀態、日期、縣市、場館或排序後，結果必須立即更新，不得出現延遲動畫造成的卡頓。
34. 使用手機版寬度檢查首頁：**循著展期，安排下一場相遇** 下方兩個日期欄必須依序堆疊並保持同一個左右邊界，不得先偏出畫面再回正。Hero 的抽卡按鈕應貼近紙張拼貼左上角，底部與第三張票券之間要保留空隙。票券第一次點擊應只把該張票券提到前方；再點同一張票券，才進入該展覽詳情。改點另一張票券時，應先切換預覽，不得立即跳頁。
...
- 強制重新整理；也可用無痕視窗開啟。
- 在網頁原始碼搜尋 `assets/app.js?v=5.0`，確認目前部署的是 V5.0。
...
若錯誤出現在 **Validate frontend, scraper rules, and published data**，並列出 `index_toplogo.png`、`top_icon_2.png` 或 `sharelogo.png`，代表目前執行的仍是 V4.3 舊清洗順序。請完整覆蓋 V5.0，提交 `Update Exhibition Hub to V5.0` 並 Push；接著回到 **Actions → Update data and deploy site → Run workflow → Branch: main → Run workflow**。不必手動編輯 `data/exhibitions.json`，V5.0 會在發布前自動移除這些介面圖片。

若紅字是 `Unable to fetch any exhibition records from the Culture Ministry feeds` 或 `Connection to cloud.culture.tw timed out`，代表文化部服務暫時無回應。V5.0 已加入既有資料備援：它會沿用上一版資料繼續補抓官方頁、測試並部署，不會因單次 API 全部逾時而中止。請先確認程式碼確實已是 V5.0，再按右上角 **Re-run jobs → Re-run all jobs**；若畫面仍顯示舊版錯誤，通常是 V5.0 尚未 Push 到 `main`。

### 顯示 `netloc ... contains invalid characters`

這代表上游單位把網址與中文聯絡資訊黏在同一個欄位，並不是你的 GitHub 操作錯誤。V5.0 已包含網址安全修正，會保留正確網域並捨棄後方說明，不會再讓整批更新中止。覆蓋 V5.0、提交並推送後，再依第七節手動執行一次工作流程即可。
...
2. 點左上方 **History**。
3. 找到 `Update Exhibition Hub to V5.0`。
