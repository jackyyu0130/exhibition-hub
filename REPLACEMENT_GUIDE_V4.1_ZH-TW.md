...
| `assets/taiwan-exhibition-journal-logo-v8.png` | `/assets/taiwan-exhibition-journal-logo-v8.png` | 新增或覆蓋 |
...
不要放成 `/Exhibition-Hub-V4.1-full-replacement/index.html`；這會讓 GitHub Pages 找不到首頁。
...
6. 打開已解壓的 V4.1 替換包，選取裡面的全部檔案與資料夾。
...
10. 回到 GitHub Desktop。左下角 **Summary (required)** 輸入：`Update Exhibition Hub to V4.1`。
...
3. 從已解壓的 V4.1 資料夾，把所有內容拖進「Drag files here」區域。務必包含 `assets`、`data`、`scripts`、`tests` 與 `.github`。
4. 等待所有檔名出現在上傳清單；向下捲到 **Commit changes**。
5. 提交訊息輸入 `Update Exhibition Hub to V4.1`。
...
`data/exhibitions.json` 會由 GitHub Actions 自動更新，因此有時會和本機 V4.1 同時修改。若 GitHub Desktop 顯示 **Resolve conflicts before Merge**，不要手動處理數百或數千個 JSON 區塊，也不要直接刪除 `<<<<<<<`、`=======`、`>>>>>>>`。

### 情況 A：你想保留剛覆蓋的 V4.1 資料
...
7. 若衝突檔是 `data/exhibitions.json`，保留本機 V4.1：
...
```bash
git commit -m "Resolve V4.1 data merge conflict"
```
...
7. 工作流程會依序進行全量官方資料補強、執行 35 項測試、上傳網站並部署 Pages。第一次全量更新可能比舊版久，請等待整個 `update-and-deploy` 變成綠色。
...
9. 首頁第一區應有淡暖灰米色滿版背景與極淡紙張網格；右側 Hero 為橫向比例，桌機高度約 470px。標題應比 V4.0 小且維持正常兩行比例；左側「收錄活動／展演場館」統計底部應與右側抽卡區對齊。
10. 左上角 Logo 只有票券內部是暖陶色；中文、英文及其他區域應透明融入導覽列底色。整體仍只比旁邊導覽文字稍大。
11. Hero 地圖應為無立體陰影的平面幾何插畫；第二張票券應高於第一張票券頂緣並露出足夠內容。票券類型應有雙層線框，條碼為深色，票面背景可看到淡色圓章浮水印。
12. **今天想看什麼？**：**查看全部展覽 ↗** 應位於日期／狀態大框最右側，呈現低彩度米棕色橢圓按鈕。
13. 場館區標題應為 **從一座場館，展開城市漫遊**。桌機一次顯示三列、每列四張矮版卡片；卡片只在固定格內由右向左揭露，不得先向左錯位再回彈。
14. 場館圖片失效時應依序切換其他館內展覽圖片，全部失效才顯示精緻分類圖，不得再出現單色大型「館」字。
15. **瀏覽所有場館 ↗** 應位於場館卡片區正下方的中央橢圓按鈕。
16. 時間區標題應為 **循著展期，安排下一場相遇**；附近展覽說明以 **讓所在的位置成為起點** 開頭。
17. 點 **探索展覽**，選取分類、縣市或場館：左上角條件標題應較小，分類之間使用頓號，各組條件以低彩度細斜線分隔，不得再出現突出的「＋」。
...
- 強制重新整理；也可用無痕視窗開啟。
- 在網頁原始碼搜尋 `assets/app.js?v=4.1`，確認目前部署的是 V4.1。
...
這代表上游單位把網址與中文聯絡資訊黏在同一個欄位，並不是你的 GitHub 操作錯誤。V4.1 已包含網址安全修正，會保留正確網域並捨棄後方說明，不會再讓整批更新中止。覆蓋 V4.1、提交並推送後，再依第七節手動執行一次工作流程即可。
...
2. 點左上方 **History**。
3. 找到 `Update Exhibition Hub to V4.1`。
