# Exhibition Hub V4.8 完整替換指南

這份替換包可直接覆蓋既有 GitHub Pages 專案。ZIP 內第一層就是網站根目錄內容；解壓後請複製「裡面的檔案」，不要把整個 ZIP 外層資料夾再包進儲存庫。

本版已完成：

- 全站改為更淡、更低飽和的暖奶茶紙張色，降低長時間閱讀負擔。
- Hero 強制回到和下方圖卡相同的 1200px 置中內容區，左右不再貼邊。
- 左側文字在內容區內再向右 12px；右側背景縮為最大 680px、高度 432px。
- 票券舞台縮為最大 590px，三張票券不再橫向過長。
- 附近展覽區改為更淺的奶茶色帶。
- 探索全台展覽的五個區塊使用不同的極淺暖色，並維持無進場動畫。
...
不要放成 `/Exhibition-Hub-V4.8-full-replacement/index.html`；這會讓 GitHub Pages 找不到首頁。
...
6. 打開已解壓的 V4.8 替換包，選取裡面的全部檔案與資料夾。
...
10. 回到 GitHub Desktop。左下角 **Summary (required)** 輸入：`Update Exhibition Hub to V4.8`。
...
3. 從已解壓的 V4.8 資料夾，把所有內容拖進「Drag files here」區域。務必包含 `assets`、`data`、`scripts`、`tests` 與 `.github`。
4. 等待所有檔名出現在上傳清單；向下捲到 **Commit changes**。
5. 提交訊息輸入 `Update Exhibition Hub to V4.8`。
...
`data/exhibitions.json` 會由 GitHub Actions 自動更新，因此有時會和本機 V4.8 同時修改。若 GitHub Desktop 顯示 **Resolve conflicts before Merge**，不要手動處理數百或數千個 JSON 區塊，也不要直接刪除 `<<<<<<<`、`=======`、`>>>>>>>`。

### 情況 A：你想保留剛覆蓋的 V4.8 資料
...
7. 若衝突檔是 `data/exhibitions.json` 與／或 `data/geocode-cache.json`，保留本機 V4.8：
...
7. 工作流程會依序進行全量官方資料補強、執行 47 項測試、上傳網站並部署 Pages。第一次全量更新可能比舊版久，請等待整個 `update-and-deploy` 變成綠色。
...
9. 首頁 Hero 主標題應分成 **展覽／是城市寫給你的信** 兩行，副標題必須在 **收錄全台展覽與演出。** 後換行。Hero 整體應和下方分類區同為 1200px 置中內容區；在約 1363px 寬的桌機畫面中，左右應各保留約 98px，而不是只有 24px。左側文字還要在內容區內再向右 12px。右側紙張拼貼最大寬度約 680px、高度約 432px，票券最大舞台寬度約 590px；背景與票券右側都必須有完整留白。
10. 左上角 Logo 只有票券內部是低彩度淺奶油棕；票型、虛線、中文字、英文及其餘透明區域不得改變。整體仍只比旁邊導覽文字稍大。
11. Hero 地圖應包含低彩度街區、河道、路徑節點、細緻建築線稿與定位點，但不得出現立體陰影或過度密集街廓。第二張票券應高於第一張票券頂緣並露出足夠內容；三張票券的中間偏右處都要有直向裁切虛線，並保留雙層類型框、深色條碼與淡色圓章浮水印。
...
16. 時間區標題應為 **循著展期，安排下一場相遇**；附近區標題應為 **離你不遠，還有這些選擇。**，說明以 **讓所在的位置成為起點** 開頭。附近區只需使用比頁面稍明顯的淡奶茶色，不得形成深色或高飽和棕色色帶。`EDITOR'S SELECTION`、`PLAN YOUR WEEK`、附近區與 `VENUE GUIDE` 上方不得出現過大的空白帶。
...
- 強制重新整理；也可用無痕視窗開啟。
- 在網頁原始碼搜尋 `assets/app.js?v=4.8`，確認目前部署的是 V4.8。
...
若錯誤出現在 **Validate frontend, scraper rules, and published data**，並列出 `index_toplogo.png`、`top_icon_2.png` 或 `sharelogo.png`，代表目前執行的仍是 V4.3 舊清洗順序。請完整覆蓋 V4.8，提交 `Update Exhibition Hub to V4.8` 並 Push；接著回到 **Actions → Update data and deploy site → Run workflow → Branch: main → Run workflow**。不必手動編輯 `data/exhibitions.json`，V4.8 會在發布前自動移除這些介面圖片。

若紅字是 `Unable to fetch any exhibition records from the Culture Ministry feeds` 或 `Connection to cloud.culture.tw timed out`，代表文化部服務暫時無回應。V4.8 已加入既有資料備援：它會沿用上一版資料繼續補抓官方頁、測試並部署，不會因單次 API 全部逾時而中止。請先確認程式碼確實已是 V4.8，再按右上角 **Re-run jobs → Re-run all jobs**；若畫面仍顯示舊版錯誤，通常是 V4.8 尚未 Push 到 `main`。

### 顯示 `netloc ... contains invalid characters`

這代表上游單位把網址與中文聯絡資訊黏在同一個欄位，並不是你的 GitHub 操作錯誤。V4.8 已包含網址安全修正，會保留正確網域並捨棄後方說明，不會再讓整批更新中止。覆蓋 V4.8、提交並推送後，再依第七節手動執行一次工作流程即可。
...
2. 點左上方 **History**。
3. 找到 `Update Exhibition Hub to V4.8`。
