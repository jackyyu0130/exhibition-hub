...
[`REPLACEMENT_GUIDE_V4.7_ZH-TW.md`](REPLACEMENT_GUIDE_V4.7_ZH-TW.md)。
...
- 全站底色校正為淡淺奶茶棕，不再呈現偏冷灰白；導覽列與卡片表面同步使用暖色象牙白。
- Hero 文案使用「展覽／是城市寫給你的信」，副標題固定分成兩行；桌機 Hero 移除過高的最小高度，右側票券背景縮回 438px。
- Hero 左右留白與下方分類區一致；右側拼貼恢復圓角長方形，網格底色由暖白漸變至低彩度木棕。
- 附近展覽大區塊使用比全站背景更明確的奶茶棕漸層，仍維持輕盈暖色。
- 探索頁的標題／狀態、分類、日期、地區場館與結果區統一為暖奶茶色系，不再使用鼠尾草綠。
