# Exhibition Hub V6.5.0-R2／台灣展覽誌

## V6.5.0-R2 累積修復版

- 將 V6.5.0 Hero／手機介面更新與 V6.5.0-R1 華山 CI 修復合併為單一安全更新包。
- 電腦版 Hero 同時顯示兩組票券＋官方圖片明信片，左箭頭下一組、右箭頭上一組。
- 移除抽卡按鈕與 15 秒自動輪播；下一組由右往左進場，原第二組移至前方。
- 手機 Hero 使用左右滑動，票券高度與標題縮小；首頁改為日曆／分類／展區／免費四入口。
- 手機探索頁的最新收錄／即將結束／免費入場徽章置中。
- 補回華山測試樣本並保留暫時逾時補抓與完整詳情安全閘門。
- 安全更新包不含展覽 JSON 與 `.github/`。

完整操作請見 [`REPLACEMENT_GUIDE_V6.5.0-R2_ZH-TW.md`](REPLACEMENT_GUIDE_V6.5.0-R2_ZH-TW.md)。
