...
不要放成 `/Exhibition-Hub-V4.3-full-replacement/index.html`；這會讓 GitHub Pages 找不到首頁。
...
6. 打開已解壓的 V4.3 替換包，選取裡面的全部檔案與資料夾。
...
10. 回到 GitHub Desktop。左下角 **Summary (required)** 輸入：`Update Exhibition Hub to V4.3`。
...
3. 從已解壓的 V4.3 資料夾，把所有內容拖進「Drag files here」區域。務必包含 `assets`、`data`、`scripts`、`tests` 與 `.github`。
4. 等待所有檔名出現在上傳清單；向下捲到 **Commit changes**。
5. 提交訊息輸入 `Update Exhibition Hub to V4.3`。
...
`data/exhibitions.json` 會由 GitHub Actions 自動更新，因此有時會和本機 V4.3 同時修改。若 GitHub Desktop 顯示 **Resolve conflicts before Merge**，不要手動處理數百或數千個 JSON 區塊，也不要直接刪除 `<<<<<<<`、`=======`、`>>>>>>>`。

### 情況 A：你想保留剛覆蓋的 V4.3 資料
...
7. 若衝突檔是 `data/exhibitions.json` 與／或 `data/geocode-cache.json`，保留本機 V4.3：
...
7. 工作流程會依序進行全量官方資料補強、執行 43 項測試、上傳網站並部署 Pages。第一次全量更新可能比舊版久，請等待整個 `update-and-deploy` 變成綠色。
...
9. 首頁第一區應有淡暖灰米色滿版背景與極淡紙張網格；主標題應為 **找一場展覽，安排今天的去處。**，右側 Hero 為橫向比例，桌機高度約 470px；左側「收錄活動／展演場館」統計底部應與右側抽卡區對齊。
...
16. 時間區標題應為 **循著展期，安排下一場相遇**；附近區標題應為 **看看附近，今天有哪些展覽。**，說明以 **讓所在的位置成為起點** 開頭。
...
- 強制重新整理；也可用無痕視窗開啟。
- 在網頁原始碼搜尋 `assets/app.js?v=4.3`，確認目前部署的是 V4.3。
...
若紅字是 `Unable to fetch any exhibition records from the Culture Ministry feeds` 或 `Connection to cloud.culture.tw timed out`，代表文化部服務暫時無回應。V4.3 已加入既有資料備援：它會沿用上一版資料繼續補抓官方頁、測試並部署，不會因單次 API 全部逾時而中止。請先確認程式碼確實已是 V4.3，再按右上角 **Re-run jobs → Re-run all jobs**；若畫面仍顯示舊版錯誤，通常是 V4.3 尚未 Push 到 `main`。

### 顯示 `netloc ... contains invalid characters`

這代表上游單位把網址與中文聯絡資訊黏在同一個欄位，並不是你的 GitHub 操作錯誤。V4.3 已包含網址安全修正，會保留正確網域並捨棄後方說明，不會再讓整批更新中止。覆蓋 V4.3、提交並推送後，再依第七節手動執行一次工作流程即可。
...
2. 點左上方 **History**。
3. 找到 `Update Exhibition Hub to V4.3`。
