...
| `assets/exhibition-fallback-sprite-v39.png` | `/assets/exhibition-fallback-sprite-v39.png` | 新增或覆蓋 |
...
不要放成 `/Exhibition-Hub-V3.9-full-replacement/index.html`；這會讓 GitHub Pages 找不到首頁。
...
6. 打開已解壓的 V3.9 替換包，選取裡面的全部檔案與資料夾。
...
10. 回到 GitHub Desktop。左下角 **Summary (required)** 輸入：`Update Exhibition Hub to V3.9`。
...
3. 從已解壓的 V3.9 資料夾，把所有內容拖進「Drag files here」區域。務必包含 `assets`、`data`、`scripts`、`tests` 與 `.github`。
4. 等待所有檔名出現在上傳清單；向下捲到 **Commit changes**。
5. 提交訊息輸入 `Update Exhibition Hub to V3.9`。
...
`data/exhibitions.json` 會由 GitHub Actions 自動更新，因此有時會和本機 V3.9 同時修改。若 GitHub Desktop 顯示 **Resolve conflicts before Merge**，不要手動處理數百或數千個 JSON 區塊，也不要直接刪除 `<<<<<<<`、`=======`、`>>>>>>>`。

### 情況 A：你想保留剛覆蓋的 V3.9 資料
...
7. 若衝突檔是 `data/exhibitions.json`，保留本機 V3.9：
...
```bash
git commit -m "Resolve V3.9 data merge conflict"
```
...
7. 工作流程會依序進行全量官方資料補強、執行 31 項測試、上傳網站並部署 Pages。第一次全量更新可能比舊版久，請等待整個 `update-and-deploy` 變成綠色。
...
9. Hero 票券背景：左上方應是精緻的城市插畫地圖，而不是簡單折線或圖表。
10. **從喜歡的場館開始探索**：每張卡應顯示場館影像或該館展覽主視覺，可用滑鼠、觸控板、手指或左右箭頭橫向瀏覽；區塊進入畫面時卡片會依序從右側緩慢滑入。
11. 展覽卡與詳情主圖：應看到清晰完整前景，後方是同一張圖的柔焦填滿背景。
12. 開啟一筆展覽詳情：標題與內文字級應比舊版小；地圖導航查詢應是場館或地址，不是活動名稱。
13. 點 **附近展覽**：瀏覽器應詢問定位權限；允許後只顯示 20 公里內活動並由近到遠排列。卡片要有綠色距離標籤與 **地圖導航 ↗**，左側地圖要顯示目前位置、20 公里範圍與活動標記。
14. 點 **我的收藏**：桌機應為每排四張 1:1 方卡，卡片由下往上緩慢浮現；有收藏時，下方要出現可橫向滑動的相似展覽推薦。
15. 左上角應顯示票券輪廓、中文「台灣展覽誌」與英文副標的平衡版 Logo；「即將開展」左側圓點應為綠色。
16. 找到沒有官方主視覺的活動：應顯示依分類製作的藝文誌風格替代圖，不再只有單色背景與大型「展」字。
17. 搜尋「吟遊於母星之間」：只能有一筆，地點為 **阿波羅畫廊**，**查看官方資訊** 應前往 `https://artemperor.tw/tidbits/19988`。
18. 抽查來源連結與圖片網址：不得出現任何 `facebook.com`、`fb.me`、`fbcdn.net` 或 `facebookusercontent.com`；也不得連到售票平台首頁。每日／手動資料更新會持續驗證售票頁與活動名稱。
...
- 強制重新整理；也可用無痕視窗開啟。
- 在網頁原始碼搜尋 `assets/app.js?v=3.9`，確認目前部署的是 V3.9。
...
2. 點左上方 **History**。
3. 找到 `Update Exhibition Hub to V3.9`。
