...
不要放成 `/Exhibition-Hub-V4.4-full-replacement/index.html`；這會讓 GitHub Pages 找不到首頁。
...
6. 打開已解壓的 V4.4 替換包，選取裡面的全部檔案與資料夾。
...
10. 回到 GitHub Desktop。左下角 **Summary (required)** 輸入：`Update Exhibition Hub to V4.4`。
...
3. 從已解壓的 V4.4 資料夾，把所有內容拖進「Drag files here」區域。務必包含 `assets`、`data`、`scripts`、`tests` 與 `.github`。
4. 等待所有檔名出現在上傳清單；向下捲到 **Commit changes**。
5. 提交訊息輸入 `Update Exhibition Hub to V4.4`。
...
`data/exhibitions.json` 會由 GitHub Actions 自動更新，因此有時會和本機 V4.4 同時修改。若 GitHub Desktop 顯示 **Resolve conflicts before Merge**，不要手動處理數百或數千個 JSON 區塊，也不要直接刪除 `<<<<<<<`、`=======`、`>>>>>>>`。

### 情況 A：你想保留剛覆蓋的 V4.4 資料
...
7. 若衝突檔是 `data/exhibitions.json` 與／或 `data/geocode-cache.json`，保留本機 V4.4：
...
7. 工作流程會依序進行全量官方資料補強、執行 45 項測試、上傳網站並部署 Pages。第一次全量更新可能比舊版久，請等待整個 `update-and-deploy` 變成綠色。
...
9. 首頁第一區應有淡暖灰米色滿版背景與極淡紙張網格；主標題應分成 **展覽／是城市寫給你的信** 兩行，副標題應為 **收錄全台展覽與演出。拆封前，先聽聽城市想說些什麼。**。右側紙張拼貼要填滿票券欄，桌機高度約 444px，只保留左側垂直分隔線；上、右、下三側不得出現外框、圓角或浮起陰影。左側「收錄活動／展演場館」統計底部應與右側抽卡區對齊。
...
12. 探索區標題應為 **循著今日心緒，遇見一場展覽**；`EXPLORE BY MOOD` 與 Hero 之間仍須能辨識段落，但不得再有過大的空白帶。狀態按鈕懸浮時應顯示淡奶黃色回饋。**查看全部展覽 ↗** 應位於日期／狀態大框最右側。
...
- 強制重新整理；也可用無痕視窗開啟。
- 在網頁原始碼搜尋 `assets/app.js?v=4.4`，確認目前部署的是 V4.4。
...
若錯誤出現在 **Validate frontend, scraper rules, and published data**，並列出 `index_toplogo.png`、`top_icon_2.png` 或 `sharelogo.png`，代表目前執行的仍是 V4.3 舊清洗順序。請完整覆蓋 V4.4，提交 `Update Exhibition Hub to V4.4` 並 Push；接著回到 **Actions → Update data and deploy site → Run workflow → Branch: main → Run workflow**。不必手動編輯 `data/exhibitions.json`，V4.4 會在發布前自動移除這些介面圖片。

若紅字是 `Unable to fetch any exhibition records from the Culture Ministry feeds` 或 `Connection to cloud.culture.tw timed out`，代表文化部服務暫時無回應。V4.4 已加入既有資料備援：它會沿用上一版資料繼續補抓官方頁、測試並部署，不會因單次 API 全部逾時而中止。請先確認程式碼確實已是 V4.4，再按右上角 **Re-run jobs → Re-run all jobs**；若畫面仍顯示舊版錯誤，通常是 V4.4 尚未 Push 到 `main`。

### 顯示 `netloc ... contains invalid characters`

這代表上游單位把網址與中文聯絡資訊黏在同一個欄位，並不是你的 GitHub 操作錯誤。V4.4 已包含網址安全修正，會保留正確網域並捨棄後方說明，不會再讓整批更新中止。覆蓋 V4.4、提交並推送後，再依第七節手動執行一次工作流程即可。
...
2. 點左上方 **History**。
3. 找到 `Update Exhibition Hub to V4.4`。
