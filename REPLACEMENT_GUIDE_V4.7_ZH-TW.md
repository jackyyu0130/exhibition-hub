# Exhibition Hub V4.7 完整替換指南

這份替換包可直接覆蓋既有 GitHub Pages 專案。ZIP 內第一層就是網站根目錄內容；解壓後請複製「裡面的檔案」，不要把整個 ZIP 外層資料夾再包進儲存庫。

本版已完成：

- 全站改為淡淺奶茶棕、暖象牙白與低彩度木棕配色。
- Hero 移除 584px 過高限制，右側票券背景縮為 438px 圓角長方形。
- Hero 左右保留和下方分類區一致的 24px 邊界，網格背景由暖白漸變至柔和木棕。
- 附近展覽區改為較明確、但不深沉的奶茶棕漸層。
- 探索全台展覽頁移除所有新增的進場與逐張圖卡動畫，分類區同步改為暖奶茶色。
...
不要放成 `/Exhibition-Hub-V4.7-full-replacement/index.html`；這會讓 GitHub Pages 找不到首頁。
...
6. 打開已解壓的 V4.7 替換包，選取裡面的全部檔案與資料夾。
...
10. 回到 GitHub Desktop。左下角 **Summary (required)** 輸入：`Update Exhibition Hub to V4.7`。
...
3. 從已解壓的 V4.7 資料夾，把所有內容拖進「Drag files here」區域。務必包含 `assets`、`data`、`scripts`、`tests` 與 `.github`。
4. 等待所有檔名出現在上傳清單；向下捲到 **Commit changes**。
5. 提交訊息輸入 `Update Exhibition Hub to V4.7`。
...
`data/exhibitions.json` 會由 GitHub Actions 自動更新，因此有時會和本機 V4.7 同時修改。若 GitHub Desktop 顯示 **Resolve conflicts before Merge**，不要手動處理數百或數千個 JSON 區塊，也不要直接刪除 `<<<<<<<`、`=======`、`>>>>>>>`。

### 情況 A：你想保留剛覆蓋的 V4.7 資料
...
7. 若衝突檔是 `data/exhibitions.json` 與／或 `data/geocode-cache.json`，保留本機 V4.7：
...
9. 首頁 Hero 主標題應分成 **展覽／是城市寫給你的信** 兩行，副標題必須在 **收錄全台展覽與演出。** 後換行。Hero 左右邊界應和下方分類區一致，不得貼到瀏覽器右緣；右側紙張拼貼應是約 438px 高的圓角長方形，四周保留留白。網格底色應由左側暖白漸變為淺奶茶色，再到右側柔和木棕；整個 Hero 不得再被 584px 最小高度撐高。
...
16. 時間區標題應為 **循著展期，安排下一場相遇**；附近區標題應為 **離你不遠，還有這些選擇。**，說明以 **讓所在的位置成為起點** 開頭。附近區背景應比全站底色更偏奶茶棕，但不得成為深棕色。`EDITOR'S SELECTION`、`PLAN YOUR WEEK`、附近區與 `VENUE GUIDE` 上方不得出現過大的空白帶。
...
31. 全站頁面底色應是明確但很淡的淺奶茶棕，而不是接近純白或冷灰；卡片表面應是暖象牙白，導覽列不得和頁面底色產生明顯色差。
32. 進入 **探索展覽**：標題／狀態、分類、日期、地區場館與展覽結果五個大區塊應使用相近的暖奶茶、淺陶土與木質紙張色；分類區不得再呈現鼠尾草綠。
...
- 強制重新整理；也可用無痕視窗開啟。
- 在網頁原始碼搜尋 `assets/app.js?v=4.7`，確認目前部署的是 V4.7。
...
若錯誤出現在 **Validate frontend, scraper rules, and published data**，並列出 `index_toplogo.png`、`top_icon_2.png` 或 `sharelogo.png`，代表目前執行的仍是 V4.3 舊清洗順序。請完整覆蓋 V4.7，提交 `Update Exhibition Hub to V4.7` 並 Push；接著回到 **Actions → Update data and deploy site → Run workflow → Branch: main → Run workflow**。不必手動編輯 `data/exhibitions.json`，V4.7 會在發布前自動移除這些介面圖片。

若紅字是 `Unable to fetch any exhibition records from the Culture Ministry feeds` 或 `Connection to cloud.culture.tw timed out`，代表文化部服務暫時無回應。V4.7 已加入既有資料備援：它會沿用上一版資料繼續補抓官方頁、測試並部署，不會因單次 API 全部逾時而中止。請先確認程式碼確實已是 V4.7，再按右上角 **Re-run jobs → Re-run all jobs**；若畫面仍顯示舊版錯誤，通常是 V4.7 尚未 Push 到 `main`。

### 顯示 `netloc ... contains invalid characters`

這代表上游單位把網址與中文聯絡資訊黏在同一個欄位，並不是你的 GitHub 操作錯誤。V4.7 已包含網址安全修正，會保留正確網域並捨棄後方說明，不會再讓整批更新中止。覆蓋 V4.7、提交並推送後，再依第七節手動執行一次工作流程即可。
...
2. 點左上方 **History**。
3. 找到 `Update Exhibition Hub to V4.7`。
