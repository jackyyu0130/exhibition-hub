# Exhibition Hub V4.9 完整替換指南

這份替換包可直接覆蓋既有 GitHub Pages 專案。ZIP 內第一層就是網站根目錄內容；解壓後請複製「裡面的檔案」，不要把整個 ZIP 外層資料夾再包進儲存庫。

本版已完成：

- Hero 的奶茶紙張色帶延伸至瀏覽器左右邊緣，文字、票券與拼貼仍維持 V4.8 的 1200px 內容位置。
- Hero 網格改為由暖白至淺木棕的低彩度漸層，城市地圖移至左上並簡化街區、路徑與建築元素。
- 「循著位置找展覽」改為較清楚但不刺眼的陶米色；左上 Logo 同步降低飽和與深度。
- 附近展覽的定位半徑由 20 公里縮為 10 公里，地圖、數量與提示文字全部共用同一設定。
- 日期／狀態展開結果回到暖象牙白，不再變成一整片淺棕色。
- 場館卡片懸浮時固定外框，只放大內部圖片。
- 手機版時間區移除左右位移，兩欄依序堆疊並維持置中。
- 手機 Hero 票券第一次點擊只展開預覽；同一張票券第二次點擊才進入展覽詳情。
...
不要放成 `/Exhibition-Hub-V4.9-full-replacement/index.html`；這會讓 GitHub Pages 找不到首頁。
...
6. 打開已解壓的 V4.9 替換包，選取裡面的全部檔案與資料夾。
...
10. 回到 GitHub Desktop。左下角 **Summary (required)** 輸入：`Update Exhibition Hub to V4.9`。
...
3. 從已解壓的 V4.9 資料夾，把所有內容拖進「Drag files here」區域。務必包含 `assets`、`data`、`scripts`、`tests` 與 `.github`。
4. 等待所有檔名出現在上傳清單；向下捲到 **Commit changes**。
5. 提交訊息輸入 `Update Exhibition Hub to V4.9`。
...
`data/exhibitions.json` 會由 GitHub Actions 自動更新，因此有時會和本機 V4.9 同時修改。若 GitHub Desktop 顯示 **Resolve conflicts before Merge**，不要手動處理數百或數千個 JSON 區塊，也不要直接刪除 `<<<<<<<`、`=======`、`>>>>>>>`。

### 情況 A：你想保留剛覆蓋的 V4.9 資料
...
7. 若衝突檔是 `data/exhibitions.json` 與／或 `data/geocode-cache.json`，保留本機 V4.9：
...
7. 工作流程會依序進行全量官方資料補強、執行 48 項測試、上傳網站並部署 Pages。第一次全量更新可能比舊版久，請等待整個 `update-and-deploy` 變成綠色。
...
4. 首頁日期／狀態：選日期，再點目前舉辦、即將舉辦等狀態，日期／狀態的大框應直接向下延伸並在框內顯示圖卡，結果位於分類列上方，不得另外跳出一個獨立區塊。卡片只會平順浮現，不應閃白或反覆消失。同一個狀態按鈕再點一次，應回到 **全部**。
...
9. 首頁 Hero 主標題應分成 **展覽／是城市寫給你的信** 兩行，副標題必須在 **收錄全台展覽與演出。** 後換行。Hero 的奶茶網格色帶應延伸到瀏覽器左右邊緣，但其中的文字、票券與拼貼位置不可跟著外移；在約 1363px 寬的桌機畫面中，內容左緣仍約在 106px。左側文字還要在內容區內再向右 12px。右側紙張拼貼最大寬度約 680px、高度約 432px，票券最大舞台寬度約 590px。
...
12. 探索區標題應為 **循著今日心緒，遇見一場展覽**；`EXPLORE BY MOOD` 與 Hero 之間仍須能辨識段落，但不得再有過大的空白帶。狀態按鈕懸浮時應顯示淡奶黃色回饋。**查看全部展覽 ↗** 應位於日期／狀態大框最右側；選取條件後，圖卡要在同一外框內由控制列下方向下展開。
...
34. 使用手機版寬度檢查首頁：**循著展期，安排下一場相遇** 下方兩個日期欄必須依序堆疊並保持同一個左右邊界，不得先偏出畫面再回正。Hero 票券第一次點擊應只把該張票券提到前方；再點同一張票券，才進入該展覽詳情。改點另一張票券時，應先切換預覽，不得立即跳頁。
...
- 強制重新整理；也可用無痕視窗開啟。
- 在網頁原始碼搜尋 `assets/app.js?v=4.9`，確認目前部署的是 V4.9。
...
若錯誤出現在 **Validate frontend, scraper rules, and published data**，並列出 `index_toplogo.png`、`top_icon_2.png` 或 `sharelogo.png`，代表目前執行的仍是 V4.3 舊清洗順序。請完整覆蓋 V4.9，提交 `Update Exhibition Hub to V4.9` 並 Push；接著回到 **Actions → Update data and deploy site → Run workflow → Branch: main → Run workflow**。不必手動編輯 `data/exhibitions.json`，V4.9 會在發布前自動移除這些介面圖片。

若紅字是 `Unable to fetch any exhibition records from the Culture Ministry feeds` 或 `Connection to cloud.culture.tw timed out`，代表文化部服務暫時無回應。V4.9 已加入既有資料備援：它會沿用上一版資料繼續補抓官方頁、測試並部署，不會因單次 API 全部逾時而中止。請先確認程式碼確實已是 V4.9，再按右上角 **Re-run jobs → Re-run all jobs**；若畫面仍顯示舊版錯誤，通常是 V4.9 尚未 Push 到 `main`。

### 顯示 `netloc ... contains invalid characters`

這代表上游單位把網址與中文聯絡資訊黏在同一個欄位，並不是你的 GitHub 操作錯誤。V4.9 已包含網址安全修正，會保留正確網域並捨棄後方說明，不會再讓整批更新中止。覆蓋 V4.9、提交並推送後，再依第七節手動執行一次工作流程即可。
...
2. 點左上方 **History**。
3. 找到 `Update Exhibition Hub to V4.9`。
