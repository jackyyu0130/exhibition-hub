...
class FrontendContractTests(unittest.TestCase):
    ...
    def test_hero_uses_manual_three_ticket_carousel(self):
        ...
        self.assertNotIn("再抽一組觀展靈感", self.html)

    def test_nearby_auto_location_radius_distance_and_external_map(self):
        self.assertIn("const NEARBY_RADIUS_KM = 3", self.app)
        ...
        self.assertIn("你附近 3 公里", self.html)
    ...
    def test_nearby_view_is_venue_led_and_routes_back_to_venue_listing(self):
        ...
        self.assertIn('3 公里內展場', self.html)
        ...
        self.assertIn("nearbyResetOriginButton", self.html)
        self.assertIn("marker.on('click'", self.app)
    ...
    def test_v45_filter_workbench_and_home_animation_reset(self):
        ...
        self.assertIn("if (!$('#heroTicketStack')?.children.length) renderHeroTickets();", self.app)
    ...
    def test_v50_neutral_hero_and_layered_filter_results(self):
        self.assertIn("Exhibition Hub V5.0", self.css)
    ...
    def test_v65_mobile_quick_filters_and_badge_alignment(self):
        self.assertIn('class="mobile-home-quick-actions"', self.html)
        self.assertIn('data-open-mobile-filter="calendar"', self.html)
        self.assertIn('data-open-mobile-filter="category"', self.html)
        self.assertIn('data-open-mobile-filter="location"', self.html)
        self.assertIn('?view=all&amp;admission=free', self.html)
        self.assertIn(".listing-view .card-badge {", self.css)
