...
| `assets/taiwan-exhibition-journal-logo-v7.png` | `/assets/taiwan-exhibition-journal-logo-v7.png` | 新增或覆蓋 |
...
不要放成 `/Exhibition-Hub-V4.0-full-replacement/index.html`；這會讓 GitHub Pages 找不到首頁。
...
6. 打開已解壓的 V4.0 替換包，選取裡面的全部檔案與資料夾。
...
10. 回到 GitHub Desktop。左下角 **Summary (required)** 輸入：`Update Exhibition Hub to V4.0`。
...
3. 從已解壓的 V4.0 資料夾，把所有內容拖進「Drag files here」區域。務必包含 `assets`、`data`、`scripts`、`tests` 與 `.github`。
4. 等待所有檔名出現在上傳清單；向下捲到 **Commit changes**。
5. 提交訊息輸入 `Update Exhibition Hub to V4.0`。
...
`data/exhibitions.json` 會由 GitHub Actions 自動更新，因此有時會和本機 V4.0 同時修改。若 GitHub Desktop 顯示 **Resolve conflicts before Merge**，不要手動處理數百或數千個 JSON 區塊，也不要直接刪除 `<<<<<<<`、`=======`、`>>>>>>>`。

### 情況 A：你想保留剛覆蓋的 V4.0 資料
...
7. 若衝突檔是 `data/exhibitions.json`，保留本機 V4.0：
...
```bash
git commit -m "Resolve V4.0 data merge conflict"
```
...
7. 工作流程會依序進行全量官方資料補強、執行 34 項測試、上傳網站並部署 Pages。第一次全量更新可能比舊版久，請等待整個 `update-and-deploy` 變成綠色。
...
4. 首頁日期／狀態：選日期，再點目前舉辦、即將舉辦等狀態，畫面只使用同一個結果區並替換內容；卡片只會平順浮現，不應閃白或反覆消失。
...
9. 首頁第一區應有淡暖灰米色滿版背景與極淡紙張網格；右側 Hero 為橫向比例，桌機高度約 470px，仍保留城市地圖、票券堆疊及凸出感。
10. 左上角票券 Logo、中文「台灣展覽誌」與英文副標應只比旁邊導覽文字稍大，不再佔據大半導覽列。
11. **今天想看什麼？**：**查看全部展覽 ↗** 應位於日期／狀態大框最右側，呈現低彩度米棕色橢圓按鈕。
12. **從喜歡的場館開始探索**：桌機一次顯示三列、每列四張矮版卡片；可用滑鼠、觸控板、手指或左右箭頭向右瀏覽更多場館。圖片失效時應依序切換其他館內展覽圖片，全部失效才顯示精緻分類圖，不得再出現單色大型「館」字。
13. **瀏覽所有場館 ↗** 應位於場館卡片區正下方的中央橢圓按鈕。
14. 展覽卡與詳情主圖：應看到清晰完整前景，後方是同一張圖的柔焦填滿背景。
15. 開啟一筆展覽詳情：標題與內文字級應比舊版小；地圖導航查詢應是場館或地址，不是活動名稱。
16. 點 **附近展覽**：瀏覽器應詢問定位權限；允許後只顯示 20 公里內活動並由近到遠排列。卡片要有綠色距離標籤與 **地圖導航 ↗**，左側地圖要顯示目前位置、20 公里範圍與活動標記。
17. 點 **我的收藏**：桌機應為每排四張 1:1 方卡，卡片由下往上緩慢浮現；有收藏時，下方標題應為 **沿著收藏，遇見下一場**，並出現可橫向滑動的相似展覽推薦。
18. 頁尾應保留探索連結，但不得再出現 **關於資料**。
19. 找到沒有官方主視覺的活動：應顯示依分類製作的藝文誌風格替代圖，不再只有單色背景與大型「展」字。
20. 搜尋「吟遊於母星之間」：只能有一筆，地點為 **阿波羅畫廊**，**查看官方資訊** 應前往 `https://artemperor.tw/tidbits/19988`。
21. 抽查來源連結與圖片網址：不得出現任何 `facebook.com`、`fb.me`、`fbcdn.net` 或 `facebookusercontent.com`；也不得連到售票平台首頁。每日／手動資料更新會持續驗證售票頁與活動名稱。
22. 搜尋公所、說故事與競賽型內容：`市公所`、`區公所`、`鄉公所`、`鎮公所`及純比賽場次應為零筆；漫畫／寵物／藝術博覽會、漫才與音樂會仍可正常保留。
...
- 強制重新整理；也可用無痕視窗開啟。
- 在網頁原始碼搜尋 `assets/app.js?v=4.0`，確認目前部署的是 V4.0。
...
這代表上游單位把網址與中文聯絡資訊黏在同一個欄位，並不是你的 GitHub 操作錯誤。V4.0 已包含網址安全修正，會保留正確網域並捨棄後方說明，不會再讓整批更新中止。覆蓋 V4.0、提交並推送後，再依第七節手動執行一次工作流程即可。
...
2. 點左上方 **History**。
3. 找到 `Update Exhibition Hub to V4.0`。
